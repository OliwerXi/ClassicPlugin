...
import club.rarlab.classicplugin.extension.capitalise
 ...
 * @see ItemStack
 */
enum class XMaterial(val data: Byte, val legacy: Array<String>) {
    ...
    companion object {
        /**
         * An immutable cached set of [XMaterial.values] to avoid allocating memory for
         * calling the method every time.
         *
         * @since 2.0.0
         */
        @JvmStatic
        val VALUES = EnumSet.allOf(XMaterial::class.java)
         ...
         * @since 3.0.0
         */
        private val DUPLICATED = EnumMap<XMaterial, XMaterial>(XMaterial::class.java).also {
            it[MELON] = MELON_SLICE
            it[CARROT] = CARROTS
            it[POTATO] = POTATOES
            it[BEETROOT] = BEETROOTS
            it[BROWN_MUSHROOM] = BROWN_MUSHROOM_BLOCK
            it[BRICK] = BRICKS
            it[NETHER_BRICK] = NETHER_BRICKS
            ...
            it[DARK_OAK_DOOR] = DARK_OAK_DOOR
            it[ACACIA_DOOR] = ACACIA_DOOR
            it[BIRCH_DOOR] = BIRCH_DOOR
            it[JUNGLE_DOOR] = JUNGLE_DOOR
            it[SPRUCE_DOOR] = SPRUCE_DOOR
            it[CAULDRON] = CAULDRON
        }
        ...
        private val NAME_CACHE = CacheBuilder.newBuilder()
                ...
                .build<String, XMaterial>()

        /**
         * Guava (Google Core Libraries for Java)'s cache for performance and timed caches.
         * For [XMaterial]s that are already parsed once.
         *
         * @since 3.0.0
         */
        private val PARSED_CACHE = CacheBuilder.newBuilder()
                .softValues()
                .expireAfterAccess(10, TimeUnit.MINUTES)
                .build<XMaterial, Optional<Material>>()
         ...
         * @since 3.0.0
         */
        private val FORMAT_PATTERN = Pattern.compile("\\W+")
        ...
        fun getMajorVersion(version: String): String {
            check(version.isNotEmpty()) { "Cannot get major minecraft version from an empty string." }
            val index = version.lastIndexOf("MC:")

            var preciseVersion: String = when {
                index != -1 -> version.substring(index + 4, version.length - 1)
                version.endsWith("SNAPSHOT") -> version.substring(0, version.indexOf('-'))
                else -> version
            }

            val lastDot = preciseVersion.lastIndexOf('.')
            if (preciseVersion.indexOf('.') != lastDot) preciseVersion = preciseVersion.substring(0, lastDot)
         ...
         */
        @JvmStatic
        val VERSION = getMajorVersion(Bukkit.getVersion()).substring(2).toInt()
         ...
         * @since 3.0.0
         */
        private val IS_FLAT = supports(13)
         ...
         */
        @JvmStatic
        val IS_NEW_VERSION: Boolean get() = IS_FLAT
         ...
         */
        @JvmStatic
        val IS_ONE_EIGHT: Boolean get() = !supports(9)

        /**
         * Gets the [XMaterial] with this name without throwing an exception.
         *
         * @param name the name of the [Material].
         * @return an optional that can be empty.
         * @since 5.1.0
         */
        private fun getIfPresent(name: String): Optional<XMaterial> = Optional.ofNullable(NAMES[name])

        /**
         * When using newer versions of Minecraft ([IS_NEW_VERSION]), helps
         ...
         * @since 1.0.0
         */
        private fun requestOldXMaterial(name: String, data: Byte): XMaterial? {
            ...
            for (material in VALUES) {
                if ((data != (-1).toByte() && data != material.data) && !material.anyMatchLegacy(name)) continue
         ...
         * @since 2.0.0
         */
        private fun format(name: String): String = FORMAT_PATTERN.matcher(
                name.trim().replace('-', '_').replace(' ', '_')
        ).replaceAll("").toUpperCase(Locale.ENGLISH)

        /**
         * Checks if [XMaterial] enum contains a material with the given name.
         * <p>
         * You should use [matchXMaterial] instead if you're going
         * to get the XMaterial object after checking if it's available in the list
         * by doing a simple [Optional.isPresent] check.
        ...
        fun contains(name: String): Boolean {
            check(name.isNotEmpty()) { "Cannot check for empty material name." }
            val preciseName = format(name)
            return VALUES.any { it.name == preciseName }
        }
         ...
         * @since 3.0.0
         */
        private fun matchDefinedXMaterial(name: String, data: Byte): Optional<XMaterial> {
            val duplicated = isDuplicated(name)

            val material = with (getIfPresent(name)) {
                if (data <= 0 && (IS_FLAT || !duplicated) && this.isPresent) return this; null
            }

            val paradoxMaterial = with (requestOldXMaterial(name, data)) {
                if (this == null) {
                    return if (data > 0 && name.endsWith("MAP")) Optional.of(FILLED_MAP) else Optional.empty()
                }; this
            }

            if (!IS_FLAT && duplicated && paradoxMaterial?.name?.get(paradoxMaterial.name.length - 1) == 'S') {
                return material ?: getIfPresent(name)
            }
         ...
         * @since 3.0.0
         */
        private fun matchXMaterialWithData(name: String): Optional<XMaterial> {
            charArrayOf(',', ':').forEach { char ->
                val index = name.indexOf(char)
                if (index == -1) return@forEach
                return matchDefinedXMaterial(
                        format(name.substring(0, index)),
                        StringUtils.deleteWhitespace(name.substring(index + 1)).toByte()
                )
            }
         ...
         */
        @JvmStatic
        fun matchXMaterial(name: String): Optional<XMaterial> {
            check(name.isNotEmpty()) { "Cannot match a material with empty material name." }
            val oldMatch = matchXMaterialWithData(name)

            if (oldMatch.isPresent) return oldMatch
            return matchDefinedXMaterial(format(name), (-1).toByte())
        }
         ...
         */
        @JvmStatic
        fun matchXMaterial(material: Material): XMaterial = matchDefinedXMaterial(material.name, (-1).toByte()).orElseThrow {
            IllegalArgumentException("Unsupported Material with no bytes: ${material.name}")
        }

        /**
         * Parses the given [ItemStack] as an [XMaterial] using its [Material] and data value (durability).
         *
         ...
         */
        @JvmStatic
        fun matchXMaterial(itemStack: ItemStack): XMaterial {
            val material = itemStack.type.name
            val data = (if (IS_FLAT || isDamageable(material)) 0 else itemStack.durability).toByte()
            return matchDefinedXMaterial(material, data).orElseThrow {
                IllegalArgumentException("Unsupported Material: $material ($data)")
            }
        }
         ...
         * <p>
         * <b>Example:</b>
         * <p>[MELON, CARROT, POTATO, BEETROOT -> true]
         *
         ...
         * @since 2.0.0
         */
        private fun isDuplicated(name: String): Boolean = DUPLICATED.entries.any {
            val material = it.key
            material.name == name || material.anyMatchLegacy(name)
        }
        ...
        fun matchXMaterial(id: Int, data: Byte): Optional<XMaterial> {
            return if (id < 0 || data < 0) Optional.empty() else Optional.ofNullable(VALUES.find { material ->
                material.data == data && material.getId() == id
            })
        }
         ...
         * @since 2.1.0
         */
        private fun toWord(name: String): String = name.replace('_', ' ').capitalise()
         ...
         */
        @JvmStatic
        fun toWord(material: Material): String = toWord(material.name)
         ...
         */
        @JvmStatic
        fun isDamageable(name: String): Boolean = DAMAGEABLE.any { name.contains(it) }
         ...
         * </pre>
         * <p>
         * The reason that there are tags for [CONTAINS] and [REGEX]
         * is for the performance.
         * Please avoid using the [REGEX] tag if you can use the [CONTAINS] tag.
         ...
         */
        @JvmStatic
        fun isOneOf(material: Material, materials: List<String>): Boolean {
            if (materials.isEmpty()) return false
            val name = material.name
            return materials.any { comp ->
                val upperCased = comp.toUpperCase()
                val contains = with (upperCased) { startsWith("CONTAINS:") && name.contains(format(substring(9))) }
                val regex = with (upperCased) { startsWith("REGEX:") && matches(substring(6).toRegex()) }
                val optional = matchXMaterial(upperCased)
                contains || regex || (optional.isPresent && optional.get().toMaterial() === material)
            }
        }
    }

    constructor(vararg legacy: String) : this(0.toByte(), arrayOf(*legacy))
    constructor(data: Int, vararg legacy: String) : this(data.toByte(), arrayOf(*legacy))
     ...
     * @since 3.0.0
     */
    fun getMaterialVersion(): Int {
        if (this.legacy.isEmpty()) return 0

        val version = this.legacy[0]
     ...
     * @since 3.0.0
     */
    override fun toString(): String = toWord(this.name)
     ...
     */
    fun getId(): Int {
        if (this.data.toInt() != 0 || (this.legacy.isNotEmpty() && this.legacy[0].substring(2).toInt() >= 13)) return -1
        return with (this.toMaterial()) { if (this == null) -1 else this.getId() }
    }
     ...
     * @since 2.0.0
     */
    fun isDuplicated(): Boolean = DUPLICATED.containsKey(this)
     ...
     * @since 1.0.0
     */
    fun isDamageable(): Boolean = Companion.isDamageable(this.name)
     ...
     * @since 2.0.0
     */
    private fun requestOldMaterial(suggest: Boolean): Material? {
        for (i in this.legacy.size - 1 downTo 0) {
            val legacy = this.legacy[i]
            if (i == 0 && legacy[1] == '.') return null
            ...
            val material = Material.getMaterial(legacy)
     ...
     * @since 2.0.0
     */
    fun toMaterial(suggest: Boolean = false): Material? {
        val cache = PARSED_CACHE.getIfPresent(this)
        if (cache != null) return cache.orElse(null)

        var material = when {
            !IS_FLAT && this.isDuplicated() -> requestOldMaterial(suggest)
            else -> {
                val mat = Material.getMaterial(this.name)
                mat ?: requestOldMaterial(suggest)
            }
        }

        if (material != null) PARSED_CACHE.put(this, Optional.ofNullable(material))
        return material
    }
     ...
     *
     * @param suggest if true [parseMaterial([Boolean])] true will be used.
     * @return an [ItemStack] with the same [Material] (and data value if in older versions.)
     * @see setType([ItemStack])
     * @since 2.0.0
     */
    fun toItem(suggest: Boolean = false): ItemStack? {
        val material = this.toMaterial(suggest) ?: return null
        return if (IS_FLAT) ItemStack(material) else ItemStack(material, 1, this.data.toShort())
    }

    /**
     * Checks if an [ItemStack] has the same [Material] (and data value on older versions).
     *
    ...
    fun isSimilar(itemStack: ItemStack): Boolean {
        if (itemStack.type != this.toMaterial()) return false
        return IS_FLAT || this.isDamageable() || itemStack.durability.toByte() == this.data
    }
    ...
    fun getSuggestions(): List<String> {
        if (this.legacy.isEmpty() || this.legacy[0][1] != '.') return emptyList()
     ...
     * Suggested [Material]s will be ignored.
     * <p>
     * Note that you should use [parseMaterial] and check if it's null
     * if you're going to parse and use the [Material] later.
     *
     * @return true if the material exists in the [Material] list.
     * @since 2.0.0
     */
    fun isSupported(): Boolean {
        val version = this.getMaterialVersion()
        if (version != 0) return supports(version)
        return Material.getMaterial(name) != null || requestOldMaterial(false) != null
    }
     ...
     * @since 2.0.0
     */
    fun isFromNewSystem(): Boolean = this.legacy.isNotEmpty() && this.legacy[0].substring(2).toInt() > 13;
}